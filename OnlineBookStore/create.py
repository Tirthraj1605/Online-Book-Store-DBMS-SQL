...
def create():
    ...
    col1=st.columns(2)
    with col1:
        Insert_data=st.text_input("Insert Data")
    if st.button("Add Tables"):
        add_data(BookId,Title,Author,Price,Quantity,UserID,Username,Password,CartID)
        st.success("Successfully added table Columns: {}".format(Title))